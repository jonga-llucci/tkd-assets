...
function submitQuestion(username, questionData) {
  ...
  newRow[0]  = questionData.question;           // Col A — question text
  newRow[4]  = questionData.correctAnswer;      // Col E — correct answer (also an option)
  newRow[5]  = questionData.opt1;               // Col F — incorrect option 1
  newRow[6]  = questionData.opt2;               // Col G — incorrect option 2
  if (questionData.opt3) newRow[7] = questionData.opt3; // Col H — incorrect option 3 (optional)
  newRow[11] = questionData.correctAnswer;      // Col L — correct answer for checking
  newRow[14] = qId;                             // Col O — unique ID
  newRow[16] = 'Y';                             // Col Q — exam eligible by default
  newRow[17] = parseInt(questionData.beltLevel); // Col R — belt level
  newRow[20] = questionData.category;           // Col U — category
  newRow[23] = questionData.clubOnly ? callerClub : ''; // Col X — club restriction
